...
import { getProfileUsername } from "../Service/Profile";
...
export const followOne = async (req: any, res: any) => {
  try {
    ...
    const updateList = await profile.updateOne(
...
export const acceptFollow = async (req: any, res: any) => {
  try {
    ...
    if (!user["friends"].includes(acceptId)) user["friends"].push(acceptId);
    else return res.send({ status: 200, message: "Already Accepted" });
...
export const searchPeople = async (req: any, res: any) => {
  try {
    const { username } = req.body;
    const result = await getProfileUsername(username);
      return res.send({
        staus: 200,
        result,
      });
